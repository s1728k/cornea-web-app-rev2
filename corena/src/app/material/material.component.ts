...
export class MaterialComponent implements OnInit, AfterViewInit {
  ...
  constructor(private restApiService: RestApiService, private spinnerloaderService: SpinnerloaderService, private dialogsService: DialogService, private loaderService: LoaderService) {
  }
  ...
  postMaterial() {
    ...
    this.restApiService.postRequestWithSpinnerLoader(url, this.newMaterial)
  ...
  generalFilter(event, n) {
    ...
    this.restApiService.getSearchRequest(url)
      ...
      .subscribe(
        (value: Material[]) => {
          this.materials_new = value;
          console.log(this.materials_new);
          this.hideLoader();
        },
        (err: any) => {
          console.error(err);
